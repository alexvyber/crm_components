defmodule CrmComponents.Typography do
  ...
  def h1(assigns) do
    ...
    <h1
      class={
        get_heading_classes(
          "text-3xl sm:text-4xl lg:text-5xl leading-10 sm:tracking-tight",
          assigns
        )
      }
      {@rest}
  ...
  def h2(assigns) do
    ...
    ~H"""
    <h2 class={get_heading_classes("text-2xl sm:text-3xl font-semibold leading-10", assigns)} {@rest}>
      <%= if assigns[:label] do %>
        <%= @label %>
      <% else %>
        <%= render_slot(@inner_block) %>
